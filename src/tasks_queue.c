...
void enqueue_task(tasks_queue_t *q, task_t *t,int th_nb) { 
    ...
    if (next == q->steal_p) {   //resizing the queue
...
{
    if (q->index == q->steal_p) { // queue empty
        return NULL;
    }
    q->index=(q->index-1)% q->task_buf_size; 
...
void steal(tasks_queue_t *q,int th_nb){ // stealing 1 thread queue and we enqueue in our queue 
